...
class BloodController extends Controller
{
    protected $web3;
    protected $contract;
    protected $contractAddress = '0x6CedEcc9BcC79CDc7935FCD738d637945CAf4A9F'; // Replace with actual contract address
    ...
    public function __construct()
    {
        ...
        $contractABI = '[{
            ...
            "inputs": [
                ...
                {"name": "_haemoglobin", "type": "uint256"},
                {"name": "_haematocrit", "type": "uint256"},
                {"name": "_rbcs", "type": "uint256"},
            ...
            "outputs": [
                {
                    "components": [
                        ...
                        {"name": "haemoglobin", "type": "uint256"},
                        {"name": "haematocrit", "type": "uint256"},
                        {"name": "rbcs", "type": "uint256"},
    ...
    public function insertBlood(Request $request)
    {
        $request->validate([
            ...
            'haematocrit' => 'required|numeric',
            'rbcs' => 'required|numeric',
        ...
        $params = [
            ...
            (int) $request->input('haemoglobin'),
            (int) $request->input('haematocrit'),
            (int) $request->input('rbcs'),
            $request->input('date')
        ];
    ...
    public function getBloodsByNationalId($nationalId)
    {
        try {
            Log::info('Request received for nationalId: ' . $nationalId);

            $this->contract->at($this->contractAddress)->call('getReportsByNationalId', (int) $nationalId, ['from' => $this->fromAddress], function ($err, $result) {
                if ($err !== null) {
                ...
                $reports = array_map(function ($report) {
                    return [
                        'nationalId' => $report[0]->value,
                        'labId' => $report[1]->value,
                        'imageUrl' => $report[2],
                        'status' => $report[3],
                        'haemoglobin' => $report[4]->value,
                        'haematocrit' => $report[5]->value,
                        'rbcs' => $report[6]->value,
                        'date' => $report[7]
                    ];
                }, $result);
